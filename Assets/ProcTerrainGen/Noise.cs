...
public static class Noise
{
    ...
    public static float[,] GenerateNoiseMap(int mapWidth, int mapHeight, int seed, float scale, int octaves, float persistence, float lacunarity, Vector2 offset, NormalizeMode normalizeMode) {
        ...
        float minLocalNoisHeight = float.MaxValue;
        ...
        for (int y = 0; y < mapHeight; y++) {
            for (int x = 0; x < mapWidth; x++) {
                ...
                else if (noiseHeight < minLocalNoisHeight) {
                    minLocalNoisHeight = noiseHeight;
                }
        ...
        for (int y = 0; y < mapHeight; y++) {
            for (int x = 0; x < mapWidth; x++) {
                if(normalizeMode == NormalizeMode.Local) {
                    noiseMap[x, y] = Mathf.InverseLerp(minLocalNoisHeight, maxLocalNoiseHeight, noiseMap[x, y]); //Keep for WATERRPG, only need one terrain chunk
                }
                else {
